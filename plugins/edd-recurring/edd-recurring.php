...
final class EDD_Recurring {
	...
	public function includes_global() {
		$files = array(
			...
			'functions.php',
			'install.php',
			'emails/edd-recurring-emails.php',
			'emails/emails.php',
		...
		// If Stripe express checkout is enabled, we reset it before getting the gateways here.
		add_filter( 'edd_get_option_stripe_prb', array( $this, 'reset_prb' ) );
		$gateways = edd_get_payment_gateways();
		remove_filter( 'edd_get_option_stripe_prb', array( $this, 'reset_prb' ) );
	...
	private function filters() {
		...
		add_filter( 'edd_deliverable_order_item_statuses', array( $this, 'edd_payment_stats_post_status' ) );
	...
	public function process_download( $download_id = 0, $email = '', $payment_id = 0, $args = array() ) {
		...
		$download_checker = new EDD\Recurring\Subscribers\DownloadChecker( $customer );
		$has_access       = $download_checker->user_can_download( $download_id, $payment_id, $price_id );
		...
		wp_die(
			sprintf(
				...
				esc_html__( 'You must have an active subscription to %s in order to download this file.', 'edd-recurring' ),
				esc_html( get_the_title( $download_id ) )
			),
			esc_html__( 'Access Denied', 'edd-recurring' )
		);
	}
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'edd-frontend-recurring', 'edd_recurring_vars', array(
			...
			'trial_message'  => __( 'Your account will be automatically charged when the free trial is completed.', 'edd-recurring' ),
		) );
 ...
 * @since v1.0
 *
 * @return EDD_Recurring|void The one true EDD_Recurring Instance (provided requirements have been met).
 */
...
\EDD\ExtensionUtils\v1\ExtensionLoader::loadOrQuit( __FILE__, 'EDD_Recurring', array(
