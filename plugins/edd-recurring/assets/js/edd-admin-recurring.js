...
jQuery( document ).ready( function ( $ ) {
	...
	var EDD_Recurring = {
		...
		edit_sub_status: function() {

			$('.edd-edit-sub-status').on('click', function(e) {
				e.preventDefault();

				var link   = $(this);
				var select = $('.edd-sub-transaction-status');
				//User clicks edit
				if (link.text() === EDD_Recurring_Vars.action_edit) {
					//Preserve current value
					link.data('current-value', select.val());
					//Update text to 'cancel'
					link.text(EDD_Recurring_Vars.action_cancel);
				} else {
					//User clicked cancel, return previous value
					select.val(link.data('current-value'));
					//Update link text back to 'edit'
					link.text(EDD_Recurring_Vars.action_edit);
				}

				select.toggle();
				$('.edd-admin-subscription-status-badge').toggle();
			});
		...
		new: function() {
			...
			$( '#edd_recurring_customer_email' ).keyup(function( e ) {
				e.preventDefault();
				if ( 13 === e.keyCode  ) {
