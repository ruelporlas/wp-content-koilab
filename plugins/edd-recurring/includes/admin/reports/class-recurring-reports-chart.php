...
class EDD_Recurring_Reports_Chart {
	...
	public function __construct() {
		if ( function_exists( '\\EDD\\Reports\\get_dates_filter_day_by_day' ) ) {
			...
			$this->converted_date = $this->get_column_conversion( 'edd_o.date_created' );
	...
	private function query() {
		...
		$sale_results = $wpdb->get_results( $wpdb->prepare(
			"SELECT COUNT(edd_o.id) AS number, SUM({$this->query_column}) AS amount, DATE_FORMAT({$this->converted_date},\"{$this->date_format}\") AS date
	 ...
	 * @return string
	 */
	private function get_column_conversion( $column = 'date_created' ) {
		if ( function_exists( 'EDD\Reports\get_column_conversion' ) ) {
			return EDD\Reports\get_column_conversion( $column );
		}
