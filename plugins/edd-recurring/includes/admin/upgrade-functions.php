...
function edd_recurring_do_automatic_upgrades() {
	...
	if ( version_compare( $version, EDD_RECURRING_VERSION, '>=' ) ) {
		return;
	}
...
function edd_recurring_update_subscription_roles() {
	if ( edd_has_upgrade_completed( 'recurring_update_subscription_roles' ) ) {
		return;
	}
...
add_action( 'admin_init', 'edd_recurring_update_subscription_roles' );
