...
class EDD_Recurring_Content_Restriction {
	...
	public function can_access_content( $return, $user_id, $restricted_to, $post_id ) {
		...
		$active_only = (bool) get_post_meta( get_the_ID(), '_edd_cr_active_only', true );
		...
		foreach ( $restricted_to as $item ) {
			...
			if ( 'any' === $item['download'] ) {
				$return['status'] = (bool) $subscriber->has_active_subscription();
			...
			if ( $recurring_enabled ) {
				...
				if ( $subscriber->has_active_product_subscription( $item['download'], $price_id ) ) {
					$has_access = true;
					break;
				}
			} else {
	...
	public function restrict_shortcode_content( $content, $restricted_to, $atts ) {
		...
		$products       = array();
		foreach ( $restricted_to as $item ) {
			$price_id = ! empty( $item['download']['price_id'] ) && is_numeric( $item['download']['price_id'] ) ? $item['download']['price_id'] : null;
			if ( edd_recurring()->is_recurring( $item['download'] ) ) {
				$has_access = $subscriber->has_active_product_subscription( $item['download'], $price_id );
			} else {
				if ( ! is_null( $price_id ) && edd_has_variable_prices( $item['download'] ) ) {
					$has_access = edd_has_user_purchased( $user_id, $item['download'], $price_id );
				} else {
