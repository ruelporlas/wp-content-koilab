		...
		foreach( $payment->cart_details as $key => $item ) {
			...
			switch( strtoupper( $_POST['message_type'] ) ) {
				...
				case 'RECURRING_INSTALLMENT_SUCCESS' :
					...
					/**
					 * First just make sure we have a timestamp value in the $_POST.
					 *
					 * Since 2Checkout doesn't send these in sandbox, we'll just make sure that we have it before we use it.
					 */
					if ( ! empty( $_POST['timestamp'] ) ) {
						...
						*/
						$subscription_payment_date = new DateTime( $_POST['timestamp'] );
					...
					$payment_id = $sub->add_payment( $subscription_payment_args );
