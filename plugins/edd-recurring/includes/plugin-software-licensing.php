...
class EDD_Recurring_Software_Licensing {
	...
	public function __construct() {
		...
		add_filter( 'edd_recurring_subscription_recurring_amounts', array( $this, 'update_recurring_amounts' ), 10, 2 );
	...
	public function update_recurring_amounts( $recurring_amounts, $item ) {
		// If this is an upgrade or a renewal, we will basically reset the recurring amounts to match the full price of the product.
		if ( ! empty( $item['item_number']['options']['is_upgrade'] ) || ! empty( $item['item_number']['options']['is_renewal'] ) ) {
			$recurring_amounts = $this->maybe_reset_recurring_amounts( $item );
		}
		...
		// Don't modify anything if there isn't an SL renewal.
		$discount = edd_sl_get_renewal_discount_percentage( 0, $item['id'] );
		if ( empty( $discount ) ) {
			return $recurring_amounts;
		}

		$tax_rate              = edd_use_taxes() && ! edd_download_is_tax_exclusive( $item['id'] ) ? edd_get_tax_rate() : 0;
		$item_price            = $recurring_amounts['amount'] / ( 1 + $tax_rate );
		$discounted_item_price = $item_price * ( 1 - ( $discount / 100 ) );
		$tax                   = $tax_rate ? $discounted_item_price * $tax_rate : 0;

		return array(
			'amount' => $discounted_item_price + $tax,
			'tax'    => $tax,
		);
	}
	...
	public function add_upgrade_and_renewal_flag( $subscription = array(), $item = array() ) {
		...
		if( isset( $item['item_number']['options']['is_upgrade'] ) ) {
			...
			$sub = $this->get_subscription_of_license( $license_id );
			if ( $sub ) {
				$subscription['old_subscription_id'] = (int) $sub->id;
			}
	...
	public function subscription_details( EDD_Subscription $subscription ) {
		...
		?>
		<div class="customer-section customer-section__license">
			<h2><?php esc_html_e( 'License Key:', 'edd-recurring' ); ?></h2>
			<p>
				<?php
				$url = false;
				...
				?>
				&mdash; <?php echo wp_kses_post( $license->get_display_status() ); ?>
			</p>
		</div>
		<?php
	}
	...
	public function checkout_upgrade_details() {
		...
		foreach ( $items as $key => $item ) {
			...
			if ( array_key_exists( $key, $cart_details ) ) {
				$subscription      = new EDD\Recurring\Cart\Subscription( $cart_details );
				$item_details      = $cart_details[ $key ];
				$recurring_amounts = $subscription->get_recurring_amount( $item_details );
				$recurring_amount  = $recurring_amounts['amount'];
			}
			if ( ! $recurring_amount ) {
				return;
			}
			...
			?>
			<div id="edd-recurring-sl-auto-renew" class="edd-alert edd-alert-warn">
				<p class="edd_error">
					<?php
					printf(
						/* translators: 1. The product name; 2. the recurring period; 3. the recurring amount. */
						esc_html__( '%1$s will now automatically renew %2$s for %3$s.', 'edd-recurring' ),
						esc_html( $this->get_item_title( $item ) ),
						esc_html( EDD_Recurring()->get_pretty_subscription_frequency( $item['options']['recurring']['period'] ) ),
						'<span class="edd-recurring-sl-discount-amount-' . esc_attr( $key ) . '">' . esc_html( edd_currency_filter( edd_sanitize_amount( $recurring_amount ) ) ) . '</span>'
					);
					?>
				</p>
			</div>
	...
	public function checkout_license_renewal_details() {
		...
		foreach ( $cart_items as $key => $item ) {
			...
			?>
			<div id="edd-recurring-sl-cancel-replace" class="edd-alert edd-alert-warn">
				<p class="edd_error">
					<?php
					printf(
						/* translators: 1. The product name; 2. the recurring period; 3. the recurring amount. */
						__( 'Your existing subscription to %1$s will be cancelled and replaced with a new subscription that automatically renews %2$s for %3$s.', 'edd-recurring' ),
						esc_html( $this->get_item_title( $item ) ),
						esc_html( EDD_Recurring()->get_pretty_subscription_frequency( $item['options']['recurring']['period'] ) ),
						'<span class="edd-recurring-sl-discount-amount-' . esc_attr( $key ) . '">' . esc_html( edd_currency_filter( edd_sanitize_amount( $recurring_amount ) ) ) . '</span>'
					);
	...
	public function update_discount_response( $data ) {
		$discounts = new EDD\Recurring\Cart\Discounts();

		return $discounts->update_discount_response( $data );
	}
	...
	private function maybe_reset_recurring_amounts( $item ) {
		...
		$subscription = new EDD\Recurring\Cart\Subscription();
		$tax_rate     = edd_use_taxes() ? edd_get_tax_rate() : 0;
		$shim_item    = wp_parse_args(
			array(
				'item_price' => floatval( $price ),
				'subtotal'   => $tax_rate && edd_prices_include_tax() ? floatval( $price / ( 1 + $tax_rate ) ) : floatval( $price ),
			),
			$item
		);

		return $subscription->calculate_recurring_amounts( $shim_item );
	}
	...
	private function get_item_price( $item ) {
		if ( edd_has_variable_prices( $item['id'] ) ) {
			return edd_get_price_option_amount( $item['id'], $item['item_number']['options']['price_id'] );
		}
	...
	private function get_item_title( $item ) {
		...
		return function_exists( 'edd_get_download_name' ) ? edd_get_download_name( $item['id'], $price_id ) : get_the_title( $item['id'] );
	}
}
