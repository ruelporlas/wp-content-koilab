...
class EDD_Recurring_Checkout {
	...
	public function init() {
		...
		add_action( 'edd_checkout_cart_item_title_after', array( $this, 'show_terms_on_cart_item' ), 10, 2 );
		...
		add_action( 'wp_ajax_edd_recurring_check_repeat_trial', array( $this, 'check_repeat_trial' ) );
	}
	...
	public function show_single_terms_notice( $download_id, $args ) {
		...
		if ( $is_recurring ) :
			...
			<p class="eddr-notice eddr-terms-notice">
				<em>
					<?php
					echo esc_html( edd_recurring_get_subscription_billing_text( $args ) );
					?>
				</em>
			</p>
			<?php
		...
		if ( $custom_recurring ) :
			...
			<p class="eddr-notice eddr-terms-notice eddr-custom-terms-notice" style="display:none">
				<em>
				<?php
					echo esc_html( edd_recurring_get_subscription_billing_text( $custom_args ) );
				?>
				</em>
			</p>
			<?php
	...
	private function get_recurring_price_text( $details ) {
		return edd_recurring_get_subscription_billing_text( $details );
	}
	...
	private function get_frequency_label( $period, $count = 1 ) {
		return edd_recurring_get_frequency_label( $period, $count );
	}
	...
	public function free_trial_total() {
		...
		if ( 1 === count( $contents ) ) {
			$subscription      = new EDD\Recurring\Cart\Subscription( $contents );
			$subscription_data = $subscription->get( reset( $contents ), 0 );
			if ( isset( $subscription_data['recurring_amount'] ) ) {
				$message = sprintf(
					...
					'<span class="edd_recurring_total_after_trial">' . edd_currency_filter( edd_sanitize_amount( $subscription_data['recurring_amount'] ) ) . '</span>'
				);
			}
		}
		?>
		<p id="edd_final_total_wrap">
			<strong><?php esc_html_e( 'Total Due Today:', 'edd-recurring' ); ?></strong>
			<span class="edd_recurring_trial_total"><?php echo esc_html( edd_currency_filter( edd_format_amount( 0.00 ) ) ); ?></span>
